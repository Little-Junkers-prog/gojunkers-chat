export default async function handler(req, res) {
  ...
  try {
    ...
    const unsafePatterns =
      /\b(stupid|dumb|idiot|fucked?|fucking|shit|bitch|damn|hell)\b/i;
    const extremeUnsafePatterns =
      /\b(kill|murder|suicide|terrorist|bomb|weapon|rape|molest)\b/i;

    const nameRegex =
      /\b(?!yard|dumpster|atlanta|peachtree|fairburn|fayetteville|newnan|tyrone|need|want|help|rental|rent|delivery|hi|hey|hello|thanks|thank|yes|no|ok|okay)([A-Z][a-z]{1,})\b/i;
    const phoneRegex = /(\d{3})[ .-]?(\d{3})[ .-]?(\d{4})/;
    const emailRegex = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i;
    const addressRegex =
      /\d{1,5}\s[A-Za-z0-9\s,.#-]+(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Drive|Dr|Court|Ct|Trail|Way|Blvd|Boulevard|Place|Pl|Parkway|Pkwy)\b/i;
    ...
    const isEndingChat =
      endOfChatSignals.test(lastUserMessage) && hasMinimumInfo;
    ...
    const askedForAddressEmailCount = messages.filter(
      (m) =>
        m.role === "assistant" &&
        /(delivery address|address|drop.?off address|email)/i.test(
          m.content || ""
        )
    ).length;
    ...
    if (extremeUnsafePatterns.test(lastUserMessage)) {
      return res.status(200).json({
        reply:
          "I'm ending this chat now. Please call us at 470-548-4733 if you need assistance. Take care.",
      });
    }
      ...
      if (hasNumber) {
        ...
        return res.status(200).json({
          reply: `I completely understand — let me have one of our team members give you a call at ${formatPhone(
            phoneMatch
          )}. They'll be able to help you better. Someone will reach out within business hours. Thanks for your patience! 👍`,
        });
      } else {
...
`;
    ...
    if (hasMinimumInfo && askedForAddressEmailCount >= 1) {
      antiLoopHints.push({
        role: "system",
        content:
          "You have name + phone/email. You have already asked for address/email once after capturing contact. Do not ask again; proceed to close politely.",
      });
    }
    ...
    return res
      .status(500)
      .json({ reply: "Server error", error: err.message });
  }
}
...
async function odooJsonRpcCall(params) {
  const url = getOdooJsonRpcUrl();
  if (!url) throw new Error("Missing Odoo JSON-RPC URL");
  ...
  if (!r.ok || !data) {
    throw new Error(`Odoo JSON-RPC HTTP error: ${r.status}`);
  }
  if (data.error) {
    throw new Error(
      `Odoo JSON-RPC error: ${data.error.message || "Unknown error"}`
    );
  }
...
async function getOdooUid() {
  ...
  if (!db || !login || !apiKey) {
    throw new Error("Missing Odoo DB/LOGIN/API_KEY env vars");
  }

  const uid = await odooJsonRpcCall({
    service: "common",
    method: "authenticate",
    args: [db, login, apiKey, {}],
  });

  if (!uid) throw new Error("Authentication failed (uid is falsy)");
  return uid;
}

async function sendLeadToOdooCRM(
  name,
  phone,
  email,
  address,
  ...
  if (!db || !apiKey || !login) {
    console.error(
      "❌ Odoo credentials (DB, LOGIN, API_KEY) are missing in environment variables. Falling back to email."
    );
    await sendLeadEmailFallback(name, phone, email, address, messages);
    return false;
  }
...
`;
  ...
  try {
    ...
    const vals = {
      ...
      type: "lead",
      // ⚠️ Only add source_id when you confirm the correct ID exists
      // source_id: 1,
    };

    const leadId = await odooJsonRpcCall({
      service: "object",
      method: "execute_kw",
      args: [db, uid, apiKey, "crm.lead", "create", [[vals]]],
    });

    console.log("✅ Lead successfully created in Odoo CRM (ID:", leadId, ")");
    return true;
  } catch (error) {
    console.error("❌ Odoo lead create failed:", error.message);
    await sendLeadEmailFallback(name, phone, email, address, messages);
    return false;
  }
}
...
async function sendLeadEmailFallback(name, phone, email, address, messages) {
  try {
    ...
    if (!r.ok) {
      ...
      console.error("❌ Resend lead fallback error:", r.status, errBody);
    }
...
async function sendEscalationEmail(name, phone, issue, messages) {
  try {
    ...
    if (!r.ok) {
      const errBody = await r.json().catch(() => null);
      console.error("❌ Resend escalation error:", r.status, errBody);
    }
